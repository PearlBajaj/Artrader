        ...
        <c:forEach items="${model.artworks}" var="artwork" varStatus="loop">
            ...
            <div class="card" style="width: 18rem;">
                ...
                <div class="card-footer empty">
                    <form method="get" action="${pageContext.request.contextPath}/sale/add">
                        <c:if test="${artwork.onSale == false}">
                            <input type="hidden" name="artworkId" value="${artwork.artworkId}">
                            <button type="submit" class="btn btn-primary btn-sm">Sell Now</button>
                        </c:if>
                        <a href="${pageContext.request.contextPath}/artwork/${artwork.artworkId}" class="float-right mt-2">See details</a>
                    </form>
                </div>
            </div>
